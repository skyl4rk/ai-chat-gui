...
MAX_MESSAGES = 5  # Keep last N messages total
...
def summarize_older_messages(api_key, or_model, messages):
    ...
    print(f"Starting summarization with {len(messages)} messages")

    if len(messages) < 3:
        print("Cannot summarize - insufficient messages")
        return messages
    ...
    if not recent_user_message or not recent_assistant_message:
        print("Cannot find recent user/assistant messages")
        return messages
    ...
    if not messages_to_summarize:
        print("No older messages to summarize")
        return messages

    print(f"Found {len(messages_to_summarize)} messages to summarize")
    ...
    print(f"Original length: {original_text_length}, Target length: {target_length}")

    try:
        print("Making summarization request...")
        ...
        print(f"Summarized content: {summarized_content[:100]}...")
        ...
        print(f"Reduced from {len(messages)} messages to {len(new_messages)} messages")
...
# Create main window
window = Tk()
window.title("Ancient Historian")
...
query_txt = scrolledtext.ScrolledText(window, width=120, height=3)
...
response_txt = scrolledtext.ScrolledText(
    window, width=120, height=20, font=("Arial", 14)
)
response_txt.configure(wrap=WORD)
